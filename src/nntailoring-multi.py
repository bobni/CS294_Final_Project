...
#keep track of number of tresholds for each class
classes = {}
class1 = ''
...
for item in sortedenergies:
    # record thresholds for each class
    if (item[1] != curclass):
        if curclass in classes:
            classes[curclass]  = classes[curclass] +1
...
# Add 1 to each class's thresholds
for class1 in classes:
    classes[class1] = classes[class1] + 1
thresholds += len(classes)
...
#let's calculate 
for class1 in classes:
    MEC += (1/len(classes)) * math.ceil(math.log(classes[class1])/math.log(2)) 
